...
def menu_main_inline_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            ...
            InlineKeyboardButton(text="🔄 Сменить план тренировок", callback_data="p:rebuild_plan"),
        ],
...
def profile_edit_field_kb(u: dict, regens_str: str = "", pending: dict | None = None) -> InlineKeyboardMarkup:
    """Меню выбора конкретного поля профиля для редактирования — 2 столбца.
    pending — dict с pending_* ключами из FSM; если есть — показывает актуальное значение в кнопках."""
    p = pending or {}

    def val(db_key: str, pending_key: str, fallback: str = "—") -> str:
        v = p[pending_key] if pending_key in p else u.get(db_key)
        return str(v) if v is not None else fallback

    act_factor = p.get("pending_activity_factor") if "pending_activity_factor" in p else u.get("activity_factor")
    ...
    rows = [
        [
            InlineKeyboardButton(text=f"🎯 Цель: {val('goal','pending_goal')}", callback_data="pf:goal"),
            InlineKeyboardButton(text=f"👤 Пол: {val('sex','pending_sex')}", callback_data="pf:sex"),
        ],
        [
            InlineKeyboardButton(text=f"🎂 Возраст: {val('age','pending_age')}", callback_data="pf:age"),
            InlineKeyboardButton(text=f"📏 Рост: {val('height','pending_height')} см", callback_data="pf:height"),
        ],
        [
            InlineKeyboardButton(text=f"⚖️ Вес: {val('weight','pending_weight')} кг", callback_data="pf:weight"),
            InlineKeyboardButton(text=f"🏠 Место: {val('place','pending_place')}", callback_data="pf:place"),
        ],
        [
            InlineKeyboardButton(text=f"📈 Опыт: {val('exp','pending_exp')}", callback_data="pf:exp"),
            InlineKeyboardButton(text=f"📅 Трен/нед: {val('freq','pending_freq')}", callback_data="pf:freq"),
        ],
        [
            InlineKeyboardButton(text=f"🏃 Активность: {_activity_label(act_factor)}", callback_data="pf:activity"),
        ],
        [
            InlineKeyboardButton(text="⛔️ Ограничения", callback_data="pf:limits"),
        ],
...
def build_day_display_text(day_num: int, day_text: str, exercises: List[str],
    ...
    # ── Блок про отдых между подходами ──────────────────────────────────────
    lines.append("⏱ Отдых между подходами")
    lines.append("")
    lines.append("🏋️ Тяжёлая база (присед, становая, жим лёжа, жим стоя)")
    lines.append("Максимальная нагрузка на нервную систему и большие мышечные группы.")
    lines.append("Отдых: 3–5 минут")
    lines.append("")
    lines.append("🔄 Средняя база (тяга верхнего блока, жим гантелей, тяга в наклоне, выпады)")
    lines.append("Многосуставные, но менее тяжёлые чем присед или становая.")
    lines.append("Отдых: 2–3 минуты")
    lines.append("")
    lines.append("💪 Изоляция на крупные мышцы (разводки, тяга на прямых руках, жим ногами)")
    lines.append("Локальная нагрузка, но мышцы крупные.")
    lines.append("Отдых: 90 секунд — 2 минуты")
    lines.append("")
    lines.append("🦾 Изоляция на малые мышцы (сгибания на бицепс, разгибания на трицепс, подъёмы на носки)")
    lines.append("Небольшие мышцы, восстанавливаются быстро.")
    lines.append("Отдых: 60–90 секунд")
    lines.append("")
    lines.append("⚡ Суперсеты на антагонисты (бицепс + трицепс, грудь + спина)")
    lines.append("Пока одна мышца работает, другая отдыхает.")
    lines.append("Отдых между парами: 60–90 секунд")
    lines.append("")
    lines.append("🔥 Дроп-сеты и подходы до полного отказа (любое упражнение)")
    lines.append("Максимальное истощение мышцы — нужно больше времени.")
    lines.append("Отдых: 2–3 минуты")
    lines.append("")
    lines.append("🤸 Упражнения с собственным весом")
    lines.append("Лёгкие (планка, скручивания, подъёмы ног) — Отдых: 45–60 секунд")
    lines.append("Средние (отжимания, приседания, выпады) — Отдых: 60–90 секунд")
    lines.append("Тяжёлые (подтягивания, отжимания на брусьях, пистолетик) — Отдых: 2–3 минуты")
    lines.append("")
    # ── Главное правило ──────────────────────────────────────────────────────
...
FAQ_QUESTIONS = {
    "faq:weight": {
        ...
        "a": (
            "⚖️ Вес стоит — разбираем причины\n\n"
            "Одна причина для любой цели: калории не совпадают с расходом.\n\n"
            "─── Самые частые причины ───\n\n"
            "1. Скрытые калории\n"
            "Соусы, масло, орехи, перекусы — всё считается.\n"
            "1 ст.л. масла = 90 ккал. 2 горсти орехов = ~300 ккал.\n"
            "За день «незаметной» еды набегает 300–600 ккал.\n\n"
            "2. Неправильное взвешивание\n"
            "Взвешивайся утром, после туалета, натощак.\n"
            "Смотри на среднее за 7 дней — не на один день.\n"
            "Колебания ±1–2 кг в течение дня — это вода, не жир.\n\n"
            "3. Слишком мало времени\n"
            "Тело меняется медленнее, чем кажется.\n"
            "Подожди минимум 2 недели, прежде чем менять калории.\n\n"
            "─── Что делать ───\n\n"
            "Хочешь похудеть, но вес стоит:\n"
            "→ Убери 150–200 ккал в день (убери перекус или уменьши порцию)\n"
            "→ Или добавь ходьбу 20–30 мин 3 раза в неделю\n"
            "→ Жди 2 недели — нет результата, убирай ещё 100 ккал\n\n"
            ...
            "После каждого изменения жди 2 недели.\n"
            "Вес — это тренд за неделю, не один день."
        )
    },
    "faq:muscle": {
        ...
        "a": (
            ...
            "Для роста нужны три вещи одновременно:\n"
            "нагрузка с прогрессией + достаточно еды + нормальный сон.\n\n"
            "─── Самые частые причины ───\n\n"
            "1. Нет прогрессии нагрузки\n"
            "Одни и те же веса месяцами — тело адаптировалось, стимула нет.\n"
            "Решение: каждые 1–2 недели добавляй +2.5 кг или +1 повторение.\n\n"
            "2. Мало белка\n"
            "На рост мышц нужно 1.8–2 г белка на кг веса.\n"
            "При 75 кг → 135–150 г белка в день.\n"
            "Пример: творог 200 г + куриное бедро 200 г + 3 яйца = ~107 г Б.\n\n"
            "3. Мало калорий\n"
            "На дефиците мышцы не растут — тело не строит то, на что нет ресурсов.\n"
            "Для набора нужен профицит +200–300 ккал/день.\n\n"
            "4. Мало сна\n"
            "Меньше 7 часов → гормон роста выделяется хуже, восстановление замедляется.\n"
            "Попробуй 2 недели по 8 часов — разница будет заметна.\n\n"
            "─── Главное ───\n\n"
            "Первые 3–6 месяцев — самый быстрый рост.\n"
            "Ключ — регулярность тренировок и постоянная прогрессия весов.\n"
            "Записывай веса в Дневник — так видишь реальный прогресс."
        )
    },
    "faq:plateau": {
    ...
    "faq:progression": {
        ...
        "a": (
            ...
            "Прогрессия — это когда каждую тренировку делаешь чуть больше.\n"
            "Без этого мышцы привыкают и перестают расти.\n\n"
            "─── Как это выглядит на практике ───\n\n"
            "Жим лёжа:\n"
            "Нед. 1: 60 кг × 8 повт.\n"
            "Нед. 2: 60 кг × 9 повт.\n"
            "Нед. 3: 62.5 кг × 8 повт.\n"
            "→ За 2 месяца рабочий вес вырос на 10 кг без форсирования.\n\n"
            "─── Три способа добавить нагрузку ───\n\n"
            "1. Добавь повторение\n"
            "Работаешь 3×6 — дойди до 3×8, затем повышай вес.\n\n"
            "2. Добавь вес\n"
            "Сделал все подходы чисто → +2.5 кг в следующую тренировку.\n\n"
            "3. Добавь подход\n"
            "Не готов к большему весу — добавь 4-й подход вместо 3.\n\n"
            "─── Когда добавлять вес ───\n\n"
            "✅ Выполнил все подходы и повторения\n"
            "✅ Техника не нарушалась\n"
            "✅ Последние повторения тяжёлые, но управляемые\n\n"
            "❌ Не добавляй вес если:\n"
            "— не добрал повторения в прошлый раз\n"
            "— техника ломается под нагрузкой\n"
            "— плохо спал или сильно болят мышцы\n\n"
            "Маленький шаг каждую неделю = большой результат за год.\n"
            "Записывай веса в Дневник — так виден реальный прогресс."
        )
    },
    "faq:diet": {
    ...
    "faq:technique": {
        ...
        "a": (
            "🏋️ Техника — зачем на неё тратить время\n\n"
            "Плохая техника = нагрузка идёт не туда или травма.\n\n"
            "─── Что происходит без техники ───\n\n"
            "• Присед с круглой спиной → болит поясница, не ноги\n"
            "• Тяга блока «руками» → работает бицепс, не спина\n"
            "• Жим с широкими локтями → болят плечи\n\n"
            "─── Как освоить технику быстро ───\n\n"
            "1. Первые 2–4 недели — 50–60% от рабочего веса\n"
            "2. Нажимай кнопку «Техника» под каждым упражнением\n"
            ...
            "• Чувствуешь именно ту мышцу, которую тренируешь\n"
            "• Нет боли в суставах после тренировки\n"
            "• Веса растут без ухудшения техники\n\n"
...
def _profile_summary_text(u: dict, pending: dict | None = None) -> str:
    """Рендерит текст профиля.
    pending — словарь с ключами вида pending_* из FSM-стейта.
    Если параметр изменён в pending — показывает новое значение и ✅."""
    p = pending or {}

    def pick(db_key: str, pending_key: str):
        """Возвращает (значение, изменено_ли)."""
        if pending_key in p:
            return p[pending_key], True
        return u.get(db_key), False

    goal,   g_ch  = pick("goal",            "pending_goal")
    sex,    s_ch  = pick("sex",             "pending_sex")
    age,    a_ch  = pick("age",             "pending_age")
    height, h_ch  = pick("height",          "pending_height")
    weight, w_ch  = pick("weight",          "pending_weight")
    place,  pl_ch = pick("place",           "pending_place")
    exp,    e_ch  = pick("exp",             "pending_exp")
    freq,   f_ch  = pick("freq",            "pending_freq")
    limits, l_ch  = pick("limits",          "pending_limits")
    act_f,  ac_ch = pick("activity_factor", "pending_activity_factor")

    def ln(label, value, changed, suffix=""):
        mark = " ✅" if changed else ""
        v = value if value is not None else "—"
        return f"{label}: {v}{suffix}{mark}"
    ...
    return "\n".join([
        ...
        ln("Пол",              sex,                     s_ch),
        ln("Возраст",          age,                     a_ch),
        ln("Рост",             height,                  h_ch,  " см"),
        ln("Вес",              weight,                  w_ch,  " кг"),
        ln("Где тренируешься", place,                   pl_ch),
        ln("Опыт",             exp,                     e_ch),
        ln("Тренировки",       freq,                    f_ch,  "×/нед"),
        ln("Активность",       _activity_label(act_f),  ac_ch),
        ln("Ограничения",      limits or "нет",         l_ch),
    ])


# =========================
# PENDING-ИЗМЕНЕНИЯ ПРОФИЛЯ
# =========================
...
async def _get_pending(state: FSMContext) -> dict:
    """Возвращает все накопленные pending_* поля из FSM-стейта."""
    data = await state.get_data()
    return {k: data[k] for k in _PENDING_KEYS if k in data}


async def _save_pending_field(state: FSMContext, **kwargs) -> None:
    """Добавляет/обновляет pending-поля, НЕ трогая остальные накопленные значения.
    Используется вместо state.clear() + state.update_data() чтобы не потерять
    уже изменённые параметры."""
    data = await state.get_data()
    # Сохраняем все уже накопленные pending-*, editing_field и прочие служебные ключи
    preserved = {k: v for k, v in data.items()
                 if k.startswith("pending_") or k == "editing_field"}
    preserved.update(kwargs)
...
async def cb_profile_edit(callback: CallbackQuery, state: FSMContext):
    ...
    # НЕ очищаем стейт — сохраняем накопленные pending-изменения
    uid = callback.from_user.id
    ...
    pending = await _get_pending(state)

    profile_text = _profile_summary_text(u, pending)
    instruction = (
        "\n\n💡 Чтобы изменить параметр — нажми на него и введи новое значение. Можно изменить несколько параметров подряд.\n"
        "Когда всё будет готово — нажми «Составить новый план»."
    )
    regens_line = f"\n\n🔄 Смен плана: {regens_str}" if regens_str else ""
    text = profile_text + instruction + regens_line
...
async def _finish_field_edit(bot: Bot, chat_id: int, user_id: int, state: FSMContext):
    """После изменения одного поля — показываем профиль с накопленными pending-значениями.
...
async def profile_field_age(message: Message, state: FSMContext, bot: Bot):
    ...
    if age is None or age < 10 or age > 90:
        ...
        await try_delete_user_message(bot, message)
        return
    await _save_pending_field(state, pending_age=age)
    await try_delete_user_message(bot, message)
...
async def profile_field_height(message: Message, state: FSMContext, bot: Bot):
    ...
    if h is None or h < 120 or h > 230:
        ...
        await try_delete_user_message(bot, message)
        return
    await _save_pending_field(state, pending_height=h)
    await try_delete_user_message(bot, message)
    await _finish_field_edit(bot, message.chat.id, message.from_user.id, state)
...
async def profile_field_weight(message: Message, state: FSMContext, bot: Bot):
    ...
    if w is None or w < 30 or w > 250:
        ...
        return
    uid = message.from_user.id
    await _save_pending_field(state, pending_weight=w)
    await try_delete_user_message(bot, message)
...
async def profile_field_limits(message: Message, state: FSMContext, bot: Bot):
    ...
    if limits.lower() in ("нет", "нету", "никаких", "no"):
        limits = ""
    await _save_pending_field(state, pending_limits=limits)
    await try_delete_user_message(bot, message)
    await _finish_field_edit(bot, message.chat.id, message.from_user.id, state)
...
async def cb_profile_goal(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if data.get("editing_field") == "goal":
        ...
        await _save_pending_field(state, pending_goal=goal)
...
async def cb_profile_sex(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if data.get("editing_field") == "sex":
        await _save_pending_field(state, pending_sex=sex)
        await _finish_field_edit(bot, callback.message.chat.id, callback.from_user.id, state)
        await callback.answer()
        return
...
async def cb_profile_place(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if data.get("editing_field") == "place":
        await _save_pending_field(state, pending_place=place)
        await _finish_field_edit(bot, callback.message.chat.id, callback.from_user.id, state)
        await callback.answer()
        return
...
async def cb_profile_exp(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if v == "0":
        ...
        if data.get("editing_field") in ("exp", "freq"):
            ...
            await _save_pending_field(state, pending_exp="0", pending_freq=3, pending_activity_factor=auto_factor)
            await _finish_field_edit(bot, callback.message.chat.id, callback.from_user.id, state)
            await callback.answer()
            return
    ...
    if data.get("editing_field") == "exp":
        await _save_pending_field(state, pending_exp=exp_text)
        await _finish_field_edit(bot, callback.message.chat.id, callback.from_user.id, state)
        await callback.answer()
        return
...
async def cb_profile_freq(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if data.get("editing_field") == "freq":
        ...
        await _save_pending_field(state, pending_freq=f, pending_activity_factor=auto_factor)
        await _finish_field_edit(bot, callback.message.chat.id, callback.from_user.id, state)
        await callback.answer()
        return
...
async def cb_profile_activity_level(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if data.get("editing_field") == "activity":
        ...
        await _save_pending_field(state, pending_activity_factor=factor)
        await _finish_field_edit(bot, callback.message.chat.id, callback.from_user.id, state)
        await callback.answer()
        return
...
MEASURE_HINTS = {
    "weight": (
        "Вставай на весы утром — после туалета, до еды и воды.\n"
        "Смотри на тренд за 1–2 недели, а не на один день:\n"
        "суточные колебания ±1–2 кг — это вода, не жир.\n"
        "Достаточно взвешиваться 2–3 раза в неделю."
    ),
    "waist": (
        "Измеряй в самом узком месте живота — чуть выше пупка.\n"
        "Встань прямо, лента горизонтально, спокойно выдохни.\n"
        "Живот не втягивай — результат будет нечестным.\n"
        "Достаточно 1 раза в неделю, в одно и то же время."
    ),
    "arm": (
        "Рука расслаблена и свисает вдоль тела.\n"
        "Лента — по середине плеча, между плечом и локтем.\n"
        "Измеряй обе руки отдельно: они могут отличаться.\n"
        "Достаточно 1 раза в неделю."
    ),
    "chest": (
        "Встань прямо, руки опусти вдоль тела.\n"
        "У женщин — по самой выступающей части груди.\n"
        "У мужчин — по линии сосков.\n"
        "Дыши спокойно, не надувай грудь. Достаточно 1 раза в неделю."
    ),
    "thigh": (
        "Встань прямо, ноги вместе.\n"
        "Лента — по самой широкой части бедра, горизонтально.\n"
        "Измеряй оба бедра: они могут отличаться.\n"
        "Достаточно 1 раза в неделю."
    ),
}
...
async def cb_measure_type(callback: CallbackQuery, state: FSMContext):
    ...
    hint = MEASURE_HINTS.get(mtype, "")
    hint_block = f"\n{hint}\n\n" if hint else "\n\n"
    text = f"📏 Замеры\n\n📝 {name}\n{hint_block}Введи значение числом:"
    back_kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def measure_value(message: Message, state: FSMContext, bot: Bot):
    ...
    if mtype == "weight":
        await update_user(uid, weight=val)
