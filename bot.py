...
TECH_VIDEOS: Dict[str, str] = {
    ...
    # "ab_rollout":          "media/tech/ab_rollout.mp4",  # удалено
    ...
    # ── Новые упражнения — раскомментируй после загрузки mp4 ────────────────
    # "squat_bw":              "media/tech/squat_bw.mp4",
...
TECH_GIFS = {
    ...
    # ── Новые домашние отжимания ──────────────────────────────────────────
    # "explosive_pushup":    "media/tech/explosive_pushup.mp4",
    # "chair_dips_pushup":   "media/tech/chair_dips_pushup.mp4",
    # "single_leg_pushup":   "media/tech/single_leg_pushup.mp4",
    # "bodyweight_hands_up_pushup": "media/tech/bodyweight_hands_up_pushup.mp4",
    # "superman":            "media/tech/superman.mp4",
    # Плечи
...
EXERCISE_NAMES = {
    ...
    "explosive_pushup":       "Взрывные отжимания",
    "chair_dips_pushup":      "Отжимания на стульях (трицепс)",
    "single_leg_pushup":      "Отжимания с поднятой ногой",
    "bodyweight_hands_up_pushup": "Bodyweight Hands Up Push Ups",
    # ── ПРЕСС / КОР ──────────────────────────────────────────────────────────
...
# =========================
# ГРУППЫ МЫШЦ И АЛЬТЕРНАТИВЫ ДЛЯ ЗАМЕНЫ УПРАЖНЕНИЙ
# =========================
# Ключ — название упражнения (как в плане), значение — (группа, [альтернативы_зал], [альтернативы_дома])
EXERCISE_ALTERNATIVES: Dict[str, Tuple[str, List[str], List[str]]] = {
    # ── КВАДРИЦЕПС / ПРИСЕД ────────────────────────────────────────────────────
    "Присед со штангой":            ("квадрицепс", ["Хакк-присед", "Жим ногами", "Гоблет-присед"], ["Приседания (свой вес)", "Болгарские выпады", "Выпады"]),
    "Присед сумо":                  ("квадрицепс", ["Хакк-присед", "Жим ногами", "Присед со штангой"], ["Приседания (свой вес)", "Выпады", "Болгарские выпады"]),
    "Хакк-присед":                  ("квадрицепс", ["Жим ногами", "Присед со штангой", "Разгибания ног в тренажёре"], ["Приседания (свой вес)", "Болгарские выпады"]),
    "Жим ногами":                   ("квадрицепс", ["Хакк-присед", "Присед со штангой", "Разгибания ног в тренажёре"], ["Приседания (свой вес)", "Болгарские выпады"]),
    "Приседания (свой вес)":        ("квадрицепс", ["Хакк-присед", "Гоблет-присед", "Жим ногами"], ["Болгарские выпады", "Выпады", "Болгарские выпады с гантелями"]),
    "Гоблет-присед":                ("квадрицепс", ["Хакк-присед", "Жим ногами", "Присед со штангой"], ["Приседания (свой вес)", "Болгарские выпады"]),
    "Разгибания ног в тренажёре":   ("квадрицепс", ["Хакк-присед", "Жим ногами"], ["Приседания (свой вес)", "Выпады"]),
    # ── ЯГОДИЦЫ / БЦБ / ВЫПАДЫ ────────────────────────────────────────────────
    "Болгарские выпады":            ("ягодицы", ["Выпады со штангой", "Жим ногами", "Хакк-присед"], ["Выпады", "Болгарские выпады с гантелями", "Приседания (свой вес)"]),
    "Болгарские выпады с гантелями":("ягодицы", ["Болгарские выпады", "Выпады с гантелями"], ["Выпады", "Приседания (свой вес)"]),
    "Выпады со штангой":            ("ягодицы", ["Болгарские выпады", "Жим ногами"], ["Выпады", "Болгарские выпады с гантелями"]),
    "Выпады с гантелями":           ("ягодицы", ["Болгарские выпады с гантелями", "Болгарские выпады"], ["Выпады", "Выпады ходьбой"]),
    "Выпады ходьбой":               ("ягодицы", ["Выпады с гантелями", "Болгарские выпады"], ["Выпады", "Болгарские выпады с гантелями"]),
    "Выпады":                       ("ягодицы", ["Болгарские выпады", "Выпады с гантелями"], ["Приседания (свой вес)", "Выпады ходьбой"]),
    "Ягодичный мост":               ("ягодицы", ["Сгибания ног", "Гиперэкстензия"], ["Ягодичный мост со своим весом", "Ягодичный мост на одной ноге", "Супермен"]),
    "Ягодичный мост со своим весом":("ягодицы", ["Ягодичный мост", "Гиперэкстензия"], ["Ягодичный мост на одной ноге", "Супермен"]),
    "Ягодичный мост на одной ноге": ("ягодицы", ["Ягодичный мост", "Гиперэкстензия"], ["Ягодичный мост со своим весом", "Супермен"]),
    # ── БИЦЕПС БЕДРА / ПОЯСНИЦА ───────────────────────────────────────────────
    "Сгибания ног":                 ("бицепс бедра", ["Румынская тяга со штангой", "Румынская тяга с гантелями"], ["Супермен", "Good Morning"]),
    "Румынская тяга со штангой":    ("бицепс бедра", ["Румынская тяга с гантелями", "Сгибания ног", "Становая тяга"], ["Супермен", "Good Morning"]),
    "Румынская тяга с гантелями":   ("бицепс бедра", ["Румынская тяга со штангой", "Сгибания ног"], ["Супермен", "Good Morning"]),
    "Становая тяга":                ("бицепс бедра", ["Становая тяга сумо", "Румынская тяга со штангой"], ["Good Morning", "Супермен"]),
    "Становая тяга сумо":           ("бицепс бедра", ["Становая тяга", "Румынская тяга со штангой"], ["Good Morning", "Супермен"]),
    "Гиперэкстензия":               ("поясница", ["Good Morning", "Румынская тяга со штангой"], ["Супермен", "Good Morning"]),
    "Супермен":                     ("поясница", ["Гиперэкстензия", "Good Morning"], ["Ягодичный мост со своим весом"]),
    "Good Morning":                 ("поясница", ["Румынская тяга со штангой", "Гиперэкстензия"], ["Супермен"]),
    # ── ГРУДЬ ─────────────────────────────────────────────────────────────────
    "Жим штанги лёжа":              ("грудь", ["Жим гантелей лёжа", "Жим в тренажёре (грудь)", "Жим под углом (штанга)"], ["Отжимания широким хватом", "Отжимания с ногами на возвышении"]),
    "Жим гантелей лёжа":            ("грудь", ["Жим штанги лёжа", "Жим в тренажёре (грудь)", "Жим под углом (гантели)"], ["Отжимания широким хватом", "Отжимания с ногами на возвышении"]),
    "Жим в тренажёре (грудь)":      ("грудь", ["Жим гантелей лёжа", "Жим штанги лёжа"], ["Отжимания широким хватом"]),
    "Жим под углом (штанга)":       ("грудь верх", ["Жим под углом (гантели)", "Жим штанги лёжа"], ["Отжимания с ногами на возвышении", "Пайк отжимания"]),
    "Жим под углом (гантели)":      ("грудь верх", ["Жим под углом (штанга)", "Жим гантелей лёжа"], ["Отжимания с ногами на возвышении", "Пайк отжимания"]),
    "Разведения на грудь":          ("грудь изоляция", ["Кроссовер", "Жим гантелей лёжа"], ["Отжимания широким хватом"]),
    "Кроссовер":                    ("грудь изоляция", ["Разведения на грудь", "Жим в тренажёре (грудь)"], ["Отжимания широким хватом"]),
    "Отжимания широким хватом":     ("грудь", ["Жим гантелей лёжа", "Жим в тренажёре (грудь)"], ["Отжимания с ногами на возвышении", "Алмазные отжимания", "Взрывные отжимания"]),
    "Отжимания с ногами на возвышении": ("грудь верх", ["Жим под углом (гантели)"], ["Пайк отжимания", "Отжимания широким хватом"]),
    "Алмазные отжимания":           ("грудь/трицепс", [], ["Отжимания узкие (трицепс)", "Отжимания широким хватом"]),
    "Пайк отжимания":               ("плечи/грудь", [], ["Отжимания с ногами на возвышении", "Взрывные отжимания"]),
    "Отжимания на брусьях":         ("грудь/трицепс", ["Жим гантелей лёжа"], ["Отжимания на стульях (трицепс)", "Алмазные отжимания"]),
    # ── СПИНА ─────────────────────────────────────────────────────────────────
    "Тяга верхнего блока широким хватом":  ("широчайшие", ["Подтягивания широким хватом", "Тяга верхнего блока узким хватом"], ["Подтягивания широким хватом", "Подтягивания обратным хватом"]),
    "Тяга верхнего блока узким хватом":    ("широчайшие", ["Тяга верхнего блока широким хватом", "Подтягивания узким хватом"], ["Подтягивания узким хватом", "Подтягивания обратным хватом"]),
    "Подтягивания широким хватом":         ("широчайшие", ["Тяга верхнего блока широким хватом", "Тяга Т-грифа"], ["Подтягивания обратным хватом", "Подтягивания узким хватом"]),
    "Подтягивания обратным хватом":        ("широчайшие", ["Тяга верхнего блока узким хватом"], ["Подтягивания широким хватом", "Подтягивания узким хватом"]),
    "Подтягивания узким хватом":           ("широчайшие", ["Тяга верхнего блока узким хватом"], ["Подтягивания обратным хватом", "Подтягивания широким хватом"]),
    "Тяга горизонтального блока":          ("спина середина", ["Тяга штанги в наклоне", "Тяга гантели", "Тяга Т-грифа"], ["Тяга гантели"]),
    "Тяга гантели":                        ("спина середина", ["Тяга горизонтального блока", "Тяга штанги в наклоне"], ["Подтягивания обратным хватом"]),
    "Тяга штанги в наклоне":               ("спина середина", ["Тяга Т-грифа", "Тяга горизонтального блока", "Тяга гантели"], ["Тяга гантели"]),
    "Тяга Т-грифа":                        ("спина середина", ["Тяга штанги в наклоне", "Тяга горизонтального блока"], ["Тяга гантели"]),
    "Face Pull":                            ("задняя дельта", ["Задняя дельта", "Задняя дельта в тренажёре"], ["Тяга гантели"]),
    "Задняя дельта":                        ("задняя дельта", ["Face Pull", "Задняя дельта в тренажёре"], ["Тяга гантели"]),
    "Задняя дельта в тренажёре":            ("задняя дельта", ["Face Pull", "Задняя дельта"], ["Тяга гантели"]),
    # ── ПЛЕЧИ ─────────────────────────────────────────────────────────────────
    "Жим штанги стоя (OHP)":        ("плечи", ["Жим гантелей стоя", "Жим гантелей сидя", "Жим в тренажёре вверх"], ["Жим гантелей стоя", "Пайк отжимания"]),
    "Жим гантелей стоя":            ("плечи", ["Жим штанги стоя (OHP)", "Жим гантелей сидя", "Арнольд-жим"], ["Жим гантелей сидя", "Пайк отжимания"]),
    "Жим гантелей сидя":            ("плечи", ["Жим гантелей стоя", "Арнольд-жим", "Жим в тренажёре вверх"], ["Жим гантелей стоя", "Пайк отжимания"]),
    "Жим в тренажёре вверх":        ("плечи", ["Жим гантелей сидя", "Жим гантелей стоя"], ["Пайк отжимания"]),
    "Арнольд-жим":                  ("плечи", ["Жим гантелей стоя", "Жим гантелей сидя"], ["Жим гантелей стоя"]),
    "Разведения гантелей в стороны":("плечи боковая", ["Подъёмы перед собой"], ["Подъёмы перед собой"]),
    "Подъёмы перед собой":          ("плечи передняя", ["Разведения гантелей в стороны"], ["Разведения гантелей в стороны"]),
    # ── БИЦЕПС ────────────────────────────────────────────────────────────────
    "Сгибания со штангой (бицепс)": ("бицепс", ["Сгибания с гантелями (бицепс)", "Молотки", "Концентрированные сгибания"], ["Сгибания с гантелями (бицепс)", "Молотки"]),
    "Сгибания с гантелями (бицепс)":("бицепс", ["Сгибания со штангой (бицепс)", "Молотки", "Концентрированные сгибания"], ["Молотки", "Концентрированные сгибания"]),
    "Молотки":                      ("бицепс", ["Сгибания с гантелями (бицепс)", "Концентрированные сгибания"], ["Сгибания с гантелями (бицепс)"]),
    "Концентрированные сгибания":   ("бицепс", ["Сгибания с гантелями (бицепс)", "Молотки"], ["Молотки"]),
    # ── ТРИЦЕПС ───────────────────────────────────────────────────────────────
    "Разгибание трицепса из-за головы":  ("трицепс", ["Разведение в блоке на трицепс", "Французский жим лёжа (гантели)"], ["Отжимания на стульях (трицепс)", "Алмазные отжимания"]),
    "Разведение в блоке на трицепс":     ("трицепс", ["Разгибание трицепса из-за головы", "Французский жим лёжа (штанга)"], ["Отжимания узкие (трицепс)", "Алмазные отжимания"]),
    "Французский жим лёжа (штанга)":    ("трицепс", ["Французский жим лёжа (гантели)", "Разведение в блоке на трицепс"], ["Отжимания узкие (трицепс)", "Алмазные отжимания"]),
    "Французский жим лёжа (гантели)":   ("трицепс", ["Французский жим лёжа (штанга)", "Разгибание трицепса из-за головы"], ["Отжимания узкие (трицепс)", "Алмазные отжимания"]),
    "Отжимания узкие (трицепс)":        ("трицепс", ["Разведение в блоке на трицепс"], ["Алмазные отжимания", "Отжимания на стульях (трицепс)"]),
    "Отжимания на стульях (трицепс)":   ("трицепс", [], ["Отжимания узкие (трицепс)", "Алмазные отжимания", "Разгибание трицепса из-за головы"]),
    # ── ПРЕСС / КОР ───────────────────────────────────────────────────────────
    "Планка (статика)":             ("кор", ["Подъёмы ног в висе", "Молитва (кранч в блоке)"], ["Боковая планка", "Скручивания (пресс)", "Велосипед"]),
    "Боковая планка":               ("кор", ["Планка (статика)", "Подъёмы ног в висе"], ["Планка (статика)", "Скручивания (пресс)"]),
    "Скручивания (пресс)":          ("пресс", ["Молитва (кранч в блоке)", "Подъёмы ног в висе"], ["Велосипед", "Русские скручивания"]),
    "Велосипед":                    ("пресс", ["Молитва (кранч в блоке)", "Подъёмы ног в висе"], ["Скручивания (пресс)", "Русские скручивания"]),
    "Русские скручивания":          ("пресс", ["Молитва (кранч в блоке)"], ["Скручивания (пресс)", "Велосипед"]),
    "Молитва (кранч в блоке)":      ("пресс", ["Подъёмы ног в висе", "Подъёмы ног в упоре на локтях"], ["Скручивания (пресс)", "Велосипед"]),
    "Подъёмы ног в висе":           ("пресс низ", ["Молитва (кранч в блоке)", "Подъёмы ног в упоре на локтях"], ["Подъёмы ног лёжа"]),
    "Подъёмы ног лёжа":             ("пресс низ", ["Подъёмы ног в висе", "Подъёмы ног в упоре на локтях"], ["Скручивания (пресс)", "Велосипед"]),
    "Подъёмы ног в упоре на локтях":("пресс низ", ["Подъёмы ног в висе", "Молитва (кранч в блоке)"], ["Подъёмы ног лёжа", "Скручивания (пресс)"]),
    # ── ИКРЫ ──────────────────────────────────────────────────────────────────
    "Подъёмы на носки в тренажёре": ("икры", ["Подъёмы на носки стоя"], ["Подъёмы на носки стоя"]),
    "Подъёмы на носки стоя":        ("икры", ["Подъёмы на носки в тренажёре"], ["Подъёмы на носки в тренажёре"]),
    # ── ИЗОЛИРУЮЩИЕ НОГИ ──────────────────────────────────────────────────────
    "Сведения ног в тренажёре":     ("приводящие", ["Отведения ног назад в кроссовере"], ["Ягодичный мост со своим весом"]),
    "Отведения ног назад в кроссовере": ("ягодицы изоляция", ["Сведения ног в тренажёре"], ["Ягодичный мост на одной ноге"]),
}


def get_exercise_alternatives(ex_name: str, place: str) -> List[str]:
    """Возвращает до 4 альтернатив для упражнения с учётом места тренировки."""
    entry = EXERCISE_ALTERNATIVES.get(ex_name)
    if not entry:
        return []
    _, gym_alts, home_alts = entry
    is_home = "дома" in (place or "").lower() or place == "свой вес"
    alts = home_alts if is_home else gym_alts
    # Если список пуст — берём из другого
    if not alts:
        alts = gym_alts if is_home else home_alts
    return alts[:4]
...
TECH = {
    ...
    "superman": {
        "title": "Супермен",
        "mp4": "media/tech/superman.mp4",
        "text": (
            "📚 Супермен (поясница + ягодицы + задняя поверхность бедра)\n\n"
            "✅ Настройка\n"
            "• Лечь лицом вниз на коврик, руки вытянуты прямо перед собой.\n"
            "• Ноги прямые, носки смотрят в пол, тело расслаблено в исходном положении.\n\n"
            "✅ Как делать\n"
            "1) На выдохе одновременно подними руки, грудь и ноги от пола.\n"
            "2) Напряги ягодицы и поясницу — тело прогибается дугой, как Superman в полёте.\n"
            "3) Зафиксируй положение 2–3 сек: руки и ноги на одной высоте.\n"
            "4) Медленно опускайся в исходное положение — 2–3 сек.\n"
            "5) Не расслабляй мышцы полностью между повторениями.\n\n"
            "⚠️ Частые ошибки\n"
            "• Поднимаешь только руки или только ноги → работают вместе одновременно.\n"
            "• Шея запрокидывается назад → держи взгляд в пол, шея нейтральна.\n"
            "• Слишком быстро без фиксации → замедлись, почувствуй пик напряжения.\n"
            "• Коленки согнуты → держи ноги прямыми на протяжении всего движения.\n\n"
            "💡 Прогрессия\n"
            "Только руки вверх → только ноги вверх → полный Superman → Superman с паузой 5 сек."
        )
    },
    "legcurl": {
        "title": "Сгибания ног",
    ...
    "ohp_dumbbell": {
        ...
        "text": (
            ...
            "✅ Настройка\n"
            "• Стой прямо, стопы на ширине плеч.\n"
            "• Гантели на уровне плеч, локти под запястьями.\n"
            "• Пресс напряжён, поясница нейтральна — не прогибайся.\n\n"
            "✅ Как делать\n"
            "1) Жми гантели вертикально вверх, слегка сближая их в верхней точке.\n"
            "2) Руки почти прямые наверху — без рывка и замка в локтях.\n"
            "3) Медленно опускай к плечам, локти под 90°.\n"
            "4) Плечи держи опущенными — не тяни к ушам.\n\n"
            "⚠️ Частые ошибки\n"
            "• Прогиб в пояснице → напряги корпус, не отклоняйся назад.\n"
            "• Гантели уходят вперёд → держи строго над плечами.\n"
            "• Читинг ногами → стой стабильно, жми только руками.\n\n"
            "💡 Плюс стоя\n"
            "Включает стабилизаторы корпуса больше, чем сидя. "
            "Независимость рук — работает на симметрию дельт."
        )
    },
    "ohp_machine": {
    ...
    "tbar_row": {
        ...
        "text": (
            "📚 Тяга Т-грифа / штанги в углу (широчайшие, ромбовидные, трапеции, бицепс)\n\n"
            "✅ Инвентарь\n"
            "• Т-гриф: используй тренажёр с грудным упором или свободностоящий.\n"
            "• Вариация со штангой: упри один конец штанги в угол стены или в специальный якорь. Нагрузи второй конец блинами.\n\n"
            "✅ Настройка\n"
            ...
            "2. Вдох — зафиксируй корпус, брюшной пресс напряжён.\n"
            ...
            "• Штанга выскальзывает из угла → надёжно зафикси упор или используй якорь.\n\n"
            "💡 Хваты\n"
            "Нейтральный (ладони друг к другу) — акцент на широчайшие.\n"
            "Пронированный (ладони вниз) — больше трапеции и задней дельты."
        )
    },
    # ── Арнольд-жим ──────────────────────────────────────────────────────────
    "arnold_press": {
    ...
    "dips": {
        "title": "Отжимания на брусьях / стульях",
        "mp4": "media/tech/dips.mp4",
        "text": (
            "📚 Отжимания на брусьях / стульях (трицепс + грудь)\n\n"
            "✅ Инвентарь\n"
            "• Брусья: хват нейтральный, тело над брусьями, руки выпрямлены.\n"
            "• Стулья / табуреты дома: поставь два устойчивых стула рядом, хват по краям сиденья.\n"
            "• Также подойдут: край дивана + стул напротив, ступени, параллельные перила.\n\n"
            "✅ Настройка\n"
            "• Акцент на трицепс: тело вертикально, локти идут строго назад вдоль корпуса.\n"
            "• Акцент на грудь: наклон вперёд ~30°, локти чуть в стороны.\n\n"
            "✅ Как делать\n"
            "1) Медленно опускайся — 2–3 сек, до 90° в локтях или ниже.\n"
            "2) Движение плавное, без раскачки и рывков.\n"
            "3) Выжимай вверх до почти выпрямленных рук — без замка в локтях.\n\n"
            "⚠️ Частые ошибки\n"
            "• Недостаточная глубина → нет нагрузки на трицепс.\n"
            "• Плечи поднимаются к ушам → держи их опущенными.\n"
            "• Стулья разъезжаются → прижми к стене или используй нескользкий пол.\n\n"
            "💡 Прогрессия\n"
            "Ноги согнуты на полу → ноги прямые → ноги на возвышении → с доп. весом."
        )
    },
    "explosive_pushup": {
        "title": "Взрывные отжимания",
        "mp4": "media/tech/explosive_pushup.mp4",
        "text": (
            "📚 Взрывные отжимания (грудь + трицепс + взрывная сила)\n\n"
            "✅ Настройка\n"
            "• Упор лёжа: ладони чуть шире плеч, тело — прямая линия.\n"
            "• Пресс и ягодицы напряжены на протяжении всего подхода.\n\n"
            "✅ Как делать\n"
            "1) Медленно опустись до касания груди пола — 2 сек.\n"
            "2) Резкий взрывной жим вверх — оттолкнись максимально сильно.\n"
            "3) Руки отрываются от пола (хлопок — по желанию).\n"
            "4) Мягко приземляйся на слегка согнутые руки — и сразу в следующее повторение.\n\n"
            "⚠️ Частые ошибки\n"
            "• Таз поднимается вверх → напряги корпус, держи прямую линию.\n"
            "• Нет медленной фазы вниз → без контроля взрыв теряет смысл.\n"
            "• Приземление на прямые руки → риск травмы запястий и локтей.\n\n"
            "💡 Прогрессия\n"
            "Обычные отжимания → взрывные без отрыва → с отрывом → с хлопком → двойной хлопок."
        )
    },
    "chair_dips_pushup": {
        "title": "Отжимания на стульях (трицепс)",
        "mp4": "media/tech/chair_dips_pushup.mp4",
        "text": (
            "📚 Отжимания на стульях — акцент на трицепс\n\n"
            "✅ Настройка\n"
            "• Два устойчивых стула / табурета на ширине плеч.\n"
            "• Хват по краям сиденья, пальцы смотрят вперёд или в стороны.\n"
            "• Тело держи вертикально — ключ к нагрузке именно на трицепс.\n\n"
            "✅ Как делать\n"
            "1) Вдох — медленно опускайся, сгибая локти строго назад (не в стороны).\n"
            "2) Опустись до угла ~90° в локтях или ниже — по самочувствию.\n"
            "3) Выдох — выжимай тело вверх силой трицепса.\n"
            "4) Наверху руки почти прямые — без полного замка в локтях.\n\n"
            "⚠️ Частые ошибки\n"
            "• Корпус наклоняется вперёд → нагрузка уходит с трицепса на грудь.\n"
            "• Стулья скользят → прижми к стене или положи на нескользкий коврик.\n"
            "• Локти расходятся в стороны → веди их строго вдоль корпуса.\n\n"
            "💡 Подсказка\n"
            "Подойдёт край дивана, кровати или ступень. Ноги прямые и дальше — сложнее. Согнуты ближе — легче."
        )
    },
    "single_leg_pushup": {
        "title": "Отжимания с поднятой ногой",
        "mp4": "media/tech/single_leg_pushup.mp4",
        "text": (
            "📚 Отжимания с поднятой ногой (грудь + кор + ягодицы)\n\n"
            "✅ Настройка\n"
            "• Обычная позиция отжиманий: ладони чуть шире плеч.\n"
            "• Подними одну ногу на 15–20 см от пола и удерживай.\n"
            "• Таз не перекашивается — бёдра параллельно полу.\n\n"
            "✅ Как делать\n"
            "1) Нога поднята — опускайся медленно (2–3 сек), грудь почти касается пола.\n"
            "2) Выжимай вверх, нога остаётся поднятой весь подход.\n"
            "3) Сделай все повторения на одну ногу, затем смени.\n\n"
            "⚠️ Частые ошибки\n"
            "• Таз уводит в сторону поднятой ноги → напряги пресс и ягодицу опорной стороны.\n"
            "• Нога слишком высоко → прогиб в пояснице, опусти ниже.\n"
            "• Скручивание плеч → держи их строго параллельно полу.\n\n"
            "💡 Прогрессия\n"
            "Обычные отжимания → с поднятой ногой → нога на возвышении + поднятая рука (T-отжимание)."
        )
    },
    "bodyweight_hands_up_pushup": {
        ...
        "text": (
            ...
            "✅ Настройка\n"
            "• Ладони строго под плечами, пальцы смотрят вперёд.\n"
            "• Спина прямая — тело единая линия от головы до пяток.\n"
            "• Пресс и ягодицы напряжены на протяжении всего подхода.\n\n"
            "✅ Как делать\n"
            "1) Медленно опускайся вниз, сводя лопатки назад и вниз — локти держи близко к корпусу.\n"
            "2) Опустись до касания грудью пола — в этот момент подними руки от пола.\n"
            "3) Зафиксируй нижнюю точку на 1 сек: грудь на полу, руки подняты, спина прямая.\n"
            "4) Верни ладони на пол и на выдохе выжми тело обратно в исходное положение.\n\n"
            "⚠️ Частые ошибки\n"
            "• Локти уходят широко в стороны → держи их под углом ~45° к корпусу.\n"
            "• Таз провисает или поднимается → напряги пресс, держи прямую линию.\n"
            "• Руки не отрываются от пола → это ключевой элемент упражнения, не пропускай его.\n"
            "• Рывок из нижней точки → касание пола должно быть контролируемым, без отбива.\n\n"
            "💡 Прогрессия\n"
            "Обычные отжимания → опускание до пола без подъёма рук → полные Hands Up Push Ups → с паузой 2–3 сек внизу."
        )
    },
    # ── СПИНА ──────────────────────────────────────────────
    # ── ПЛЕЧИ ──────────────────────────────────────────────
...
EXERCISE_TECH_MAP = [
    ...
    ("велосипед", "bicycle_crunch"),
    ("скручивания (пресс)", "ab_crunch"),
    ...
    # ── НОГИ (ТРЕНАЖЁРЫ) ────────────────────────────────────────────────────
    ("разгибания ног в тренажёре", "leg_extension"),
    ("разгибание ног в тренажёре", "leg_extension"),
    ("сведения ног в тренажёре", "leg_adduction"),
    ("отведения ног назад в кроссовере", "cable_kickback"),
    ("кроссовер отведени", "cable_kickback"),
    ...
    ("подъёмы на носки", "calves_standing"),

    # ── ГРУДЬ (ЖИМ ЛЁЖА) — специфичные перед общим "жим" ───────────────────
    ("жим штанги под углом", "incline_press_barbell"),
    ("жим под углом (штанга)", "incline_press_barbell"),
    ("жим гантелей под углом", "incline_press_dumbbell"),
    ("жим под углом (гантели)", "incline_press_dumbbell"),
    ("жим под углом", "incline_press_barbell"),
    ("жим гантелей лёжа", "bench_dumbbell"),
    ("жим гантелей лёж", "bench_dumbbell"),
    ...
    # ── ПЛЕЧИ (ЖИМ ВВЕРХ) — специфичные перед общим "жим" ──────────────────
    ("армейский жим", "ohp_barbell"),
    ("жим штанги стоя (ohp)", "ohp_barbell"),
    ("жим штанги стоя", "ohp_barbell"),
    ("арнольд-жим", "arnold_press"),
    ("арнольд жим", "arnold_press"),
    ("жим арнольда", "arnold_press"),
    ("жим гантелей стоя", "ohp_dumbbell"),
    ("жим гантелей сидя", "ohp_dumbbell_sitting"),
    ("жим в тренажёре вверх", "ohp_machine"),
    ("жим вверх", "ohp_dumbbell"),

    # ── ОТЖИМАНИЯ — специфичные перед общим "отжима" ────────────────────────
    ...
    ("на брусьях", "dips"),
    ("взрывные отжимания", "explosive_pushup"),
    ("отжимания на стульях", "chair_dips_pushup"),
    ("отжимания с поднятой ногой", "single_leg_pushup"),
    ("bodyweight hands up", "bodyweight_hands_up_pushup"),
    ("hands up push", "bodyweight_hands_up_pushup"),
    ("отжимания", "pushup_wide"),

    # ── ПОДТЯГИВАНИЯ — специфичные перед общим "подтяг" ─────────────────────
    ("подтягивания широким хватом", "pullup_wide"),
    ("подтягивания (широкий хват)", "pullup_wide"),
    ("подтягивания (широкий", "pullup_wide"),
    ...
    # ── ТЯГА ВЕРХНЕГО БЛОКА ─────────────────────────────────────────────────
    ("тяга верхнего блока широким хватом", "latpulldown_wide"),
    ("тяга верхнего блока широк", "latpulldown_wide"),
    ("верхний блок широк", "latpulldown_wide"),
    ("тяга верхнего блока узким хватом", "latpulldown_narrow"),
    ("тяга верхнего блока узк", "latpulldown_narrow"),
    ("верхний блок узк", "latpulldown_narrow"),

    # ── ГОРИЗОНТАЛЬНЫЕ ТЯГИ ─────────────────────────────────────────────────
    ("тяга штанги в наклоне", "barbell_row"),
    ("тяга т-грифа", "tbar_row"),
    ("тяга т-гриф", "tbar_row"),
    ...
    ("подъёмы перед собой", "front_raise"),
    ("подъём перед собой", "front_raise"),

    # ── БИЦЕПС ──────────────────────────────────────────────────────────────
    ("концентрированные сгибания", "concentration_curl"),
    ("сгибания со штангой (бицепс)", "biceps_barbell"),
    ("сгибания штанги стоя", "biceps_barbell"),
    ("сгибания со штангой", "biceps_barbell"),
    ("сгибания с гантелями (бицепс)", "biceps_dumbbell"),
    ("сгибания гантелей стоя", "biceps_dumbbell"),
    ("сгибания с гантелями", "biceps_dumbbell"),
    ...
    ("трицепс из-за головы", "triceps_oh"),
...
def get_tech_key_for_exercise(name: str) -> Optional[str]:
    ...
    for keyword, tech_key in EXERCISE_TECH_MAP:
        if keyword in n:
            return tech_key
    # Fallback — только реальные ключи из TECH
    if any(x in n for x in ["приседани", "присед"]):
        return "squat_bw"
    if any(x in n for x in ["выпад"]):
        return "lunge_bw"
    if any(x in n for x in ["ягодиц"]):
        return "glute_bridge"
    if any(x in n for x in ["икр", "носк"]):
        return "calves_standing"
    if any(x in n for x in ["бедр", "ног"]):
        return "legpress"
    if any(x in n for x in ["жим лёж", "жим леж"]):
        return "bench_barbell"
    if any(x in n for x in ["грудь", "грудн"]):
        return "chest_fly"
    if any(x in n for x in ["подтяг"]):
        return "pullup_wide"
    if any(x in n for x in ["блок", "тяг"]):
        return "latpulldown_wide"
    if any(x in n for x in ["спин"]):
        return "dumbbell_row"
    if any(x in n for x in ["плеч", "дельт"]):
        return "ohp_dumbbell"
    if any(x in n for x in ["бицепс", "сгибан"]):
        return "biceps_barbell"
    if any(x in n for x in ["трицепс", "разгибан"]):
        return "triceps_cable"
    if any(x in n for x in ["планк"]):
        return "planks_static"
    if any(x in n for x in ["пресс", "скручив"]):
        return "ab_crunch"
    ...
    if any(x in n for x in ["румынск", "мертвая"]):
        return "rdl_barbell"
    if any(x in n for x in ["становая"]):
        return "deadlift"
    return None
...
def generate_workout_plan(goal: str, place: str, exp: str, freq: int, limits: str, user_id: int = 0) -> Tuple[str, dict]:
    ...
    if is_gym:
        ...
        BASE_HPUSH = [
            "Взрывные отжимания",                    # explosive_pushup ★
            "Отжимания с поднятой ногой",            # single_leg_pushup
            "Bodyweight Hands Up Push Ups",          # bodyweight_hands_up_pushup
            "Отжимания широким хватом",              # pushup_wide
            "Отжимания с ногами на возвышенности",   # pushup_elevated
        ]
        BASE_HPULL = [
        ...
        ISO_TRI = [
            "Отжимания на стульях (трицепс)", # chair_dips_pushup
            "Отжимания узкие (трицепс)",      # narrow_pushup
            "Алмазные отжимания",             # diamond_pushup
            "Разгибание гантели из-за головы",# triceps_oh
        ]
...
async def cb_profile_activity_level(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if data.get("editing_field") == "activity":
        # Редактирование из профиля — возвращаем обратно в меню профиля
        await state.clear()
        await _finish_field_edit(bot, callback.message.chat.id, callback.from_user.id)
        await callback.answer()
        return
...
def workout_day_exercises_kb(day: int, exercises: List[str], done: List[int]) -> InlineKeyboardMarkup:
    """Клавиатура упражнений дня с чекбоксами, кнопкой техники и кнопкой замены."""
    rows = []
    for idx, name in enumerate(exercises):
        ...
        replace_btn = InlineKeyboardButton(
            text="🔄",
            callback_data=f"wex:swap:{day}:{idx}"
        )
        if tech_key:
            ...
            rows.append([done_btn, tech_btn, replace_btn])
        else:
            rows.append([done_btn, replace_btn])
...
async def cb_workout_ex_swap(callback: CallbackQuery, bot: Bot):
    """Показываем список альтернативных упражнений для замены.
    callback_data: wex:swap:{day_num}:{ex_idx}
    """
    parts = callback.data.split(":")
    day_num = int(parts[2])
    ex_idx = int(parts[3])
    ...
    alts = get_exercise_alternatives(ex_name, place)

    if not alts:
        await callback.answer(
            f"Для «{ex_name}» альтернативы не найдены 😔",
            show_alert=True
        )
        return

    # Убираем уже присутствующие в плане упражнения
    all_ex_in_day = set(exercises)
    alts = [a for a in alts if a not in all_ex_in_day] or alts

    entry = EXERCISE_ALTERNATIVES.get(ex_name)
    muscle = entry[0] if entry else "мышцы"

    short = ex_name[:30] + "…" if len(ex_name) > 30 else ex_name
    text = (
        f"🔄 Замена: {short}\n"
        f"📌 Группа: {muscle}\n\n"
    ...
    rows = []
    for alt in alts[:4]:
        alt_short = alt[:35] + "…" if len(alt) > 35 else alt
        rows.append([InlineKeyboardButton(
            text=alt_short,
            callback_data=f"wex:swap_confirm:{day_num}:{ex_idx}:{alt[:60]}"
        )])
    rows.append([InlineKeyboardButton(
        text="❌ Отмена",
...
async def cb_workout_ex_swap_confirm(callback: CallbackQuery, bot: Bot):
    """Подтверждаем замену и сохраняем обновлённый план.
    callback_data: wex:swap_confirm:{day_num}:{ex_idx}:{new_ex_name}
    """
    ...
    old_ex = exercises[ex_idx]

    # Заменяем строку в тексте дня
    # Формат строки: "• Название — 3×10" или "• Название"
    import re
    old_line_pattern = re.compile(
        r'(•\s*)' + re.escape(old_ex) + r'(\s*(?:—.*)?)',
        re.MULTILINE
    )
    new_day_text = old_line_pattern.sub(r'\g<1>' + new_ex + r'\g<2>', day_text, count=1)

    if new_day_text == day_text:
        await callback.answer("Не удалось заменить 😔", show_alert=True)
        return
    ...
    # Сбрасываем прогресс текущего дня (упражнение изменилось)
    await set_day_done_exercises(uid, day_num, [])
    ...
    await clean_edit(callback, uid, text, reply_markup=kb)
    await callback.answer(f"✅ Заменено на «{new_ex[:25]}»")
...
def setup_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(cb_workout_ex_swap, F.data.startswith("wex:swap:"))
    dp.callback_query.register(cb_workout_ex_swap_confirm, F.data.startswith("wex:swap_confirm:"))
