...
TECH_GIFS = {
    ...
    # "reach_pushup":        "media/tech/reach_pushup.mp4",
...
EXERCISE_NAMES = {
    ...
    "reach_pushup":           "Отжимания с подъёмом руки вверх",
    # ── ПРЕСС / КОР ──────────────────────────────────────────────────────────
...
TECH = {
    ...
    "reach_pushup": {
        "title": "Отжимания с подъёмом руки вверх",
        "mp4": "media/tech/reach_pushup.mp4",
        "text": (
            "📚 Отжимания с подъёмом руки вверх (грудь + плечи + кор)\n\n"
            "✅ Настройка\n"
            "• Упор лёжа: ладони чуть шире плеч, ноги чуть шире обычного — для баланса.\n\n"
            "✅ Как делать\n"
            "1) Опустись в отжимание — грудь почти касается пола.\n"
            "2) В верхней точке подними одну руку прямо перед собой или в сторону.\n"
            "3) Зафиксируй 1–2 сек — почувствуй работу кора и плеча.\n"
            "4) Верни руку на пол — следующее отжимание с другой рукой.\n"
            "5) Чередуй руки каждое повторение.\n\n"
            "⚠️ Частые ошибки\n"
            "• Корпус поворачивается вслед за рукой → плечи параллельно полу.\n"
            "• Таз поднимается при подъёме руки → напряги пресс сильнее.\n"
            "• Слишком быстро → сначала освой медленный контролируемый темп.\n\n"
            "💡 Прогрессия\n"
            "Обычные отжимания → подъём руки в сторону → T-отжимание (рука + разворот корпуса)."
        )
    },
    # ── СПИНА ──────────────────────────────────────────────
    # ── ПЛЕЧИ ──────────────────────────────────────────────
...
EXERCISE_TECH_MAP = [
    ...
    ("отжимания с подъёмом руки", "reach_pushup"),
...
def generate_workout_plan(goal: str, place: str, exp: str, freq: int, limits: str, user_id: int = 0) -> Tuple[str, dict]:
    ...
    if is_gym:
        ...
        BASE_HPUSH = [
            "Взрывные отжимания",                    # explosive_pushup ★
            "Отжимания с поднятой ногой",            # single_leg_pushup
            "Отжимания с подъёмом руки вверх",       # reach_pushup
            "Отжимания широким хватом",              # pushup_wide
            "Отжимания с ногами на возвышенности",   # pushup_elevated
        ]
        BASE_HPULL = [
