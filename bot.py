...
def build_three_day_menu(calories: int, meals: int) -> str:
    """
    ✅ НОВОЕ: Примеры на 3 дня (заранее заменяя блюда).
    Кол-во приёмов пищи: 3/4/5 — подстраивается.
    Все граммовки масштабируются от калорий.
    """
    k = max(calories / 2500, 0.7)

    # базовые "кирпичи"
    oat = _round_g(80 * k)                 # овсянка (сухая)
    milk = _round_g(250 * k, 50)           # молоко/кефир мл
    banana = _round_g(120 * k)
    berries = _round_g(150 * k)

    rice_cooked = _round_g(250 * k, 10)    # рис ГОТОВЫЙ
    buckwheat_cooked = _round_g(260 * k, 10)
    pasta_cooked = _round_g(260 * k, 10)
    potato = _round_g(350 * k, 10)

    chicken = _round_g(200 * k, 10)
    turkey = _round_g(200 * k, 10)
    fish = _round_g(220 * k, 10)

    veg = _round_g(300 * k, 10)
    oil = _round_g(12 * k, 1)              # масло (важно считать!)
    nuts = _round_g(20 * k, 5)

    curd = _round_g(250 * k, 10)           # творог
    yogurt = _round_g(300 * k, 10)         # йогурт/кефир
    bread = _round_g(100 * k, 10)
    eggs = max(2, int(round(3 * k)))       # шт

    extra_carb = _round_g(60 * k, 10)      # + угли
    extra_prot = _round_g(150 * k, 10)     # + белок

    def day_block(day_no: int, items: list[str]) -> str:
        return f"🗓 День {day_no}\n" + "\n".join([f"{i+1}) {x}" for i, x in enumerate(items)])

    def format_by_meals(d3, d4, d5):
        if meals == 3:
            return d3
        if meals == 4:
            return d4
    ...
    # ---------- День 1 ----------
    d1_3 = [
        f"Завтрак: овсянка {oat}г (сухая) + молоко {milk}мл + банан {banana}г",
        f"Обед: рис {rice_cooked}г (готовый) + курица {chicken}г + овощи {veg}г + масло {oil}г",
        f"Ужин: творог {curd}г + орехи {nuts}г",
    ]
    d1_4 = [
        f"Завтрак: овсянка {oat}г (сухая) + молоко {milk}мл + банан {banana}г",
        f"Перекус: йогурт/кефир {yogurt}г + ягоды {berries}г",
        f"Обед: рис {rice_cooked}г (готовый) + курица {chicken}г + овощи {veg}г + масло {oil}г",
        f"Ужин: творог {curd}г + орехи {nuts}г",
    ]
    d1_5 = [
        f"Завтрак: овсянка {oat}г (сухая) + молоко {milk}мл",
        f"Перекус: яйца ~{eggs} шт + хлеб {bread}г + овощи 200г",
        f"Обед: рис {rice_cooked}г (готовый) + курица {chicken}г + овощи {veg}г + масло {oil}г",
        f"Перекус: йогурт/кефир {yogurt}г + банан {banana}г",
        f"Ужин: творог {curd}г + (если нужно добрать) +{extra_carb}г углеводов или +{extra_prot}г белка",
    ]

    # ---------- День 2 (замены) ----------
    d2_3 = [
        f"Завтрак: яйца ~{eggs} шт + хлеб {bread}г + овощи 200г",
        f"Обед: гречка {buckwheat_cooked}г (готовая) + индейка {turkey}г + овощи {veg}г + масло {oil}г",
        f"Ужин: йогурт/кефир {yogurt}г + орехи {nuts}г",
    ]
    d2_4 = [
        f"Завтрак: яйца ~{eggs} шт + хлеб {bread}г + овощи 200г",
        f"Перекус: творог {curd}г + ягоды {berries}г",
        f"Обед: гречка {buckwheat_cooked}г (готовая) + индейка {turkey}г + овощи {veg}г + масло {oil}г",
        f"Ужин: йогурт/кефир {yogurt}г + орехи {nuts}г",
    ]
    d2_5 = [
        ...
        f"Перекус: йогурт/кефир {yogurt}г + ягоды {berries}г",
        f"Обед: гречка {buckwheat_cooked}г (готовая) + индейка {turkey}г + овощи {veg}г + масло {oil}г",
        f"Перекус: банан {banana}г + (по желанию) молоко/кефир {milk}мл",
        f"Ужин: творог {curd}г + (если нужно добрать) +{extra_carb}г углеводов или +{extra_prot}г белка",
    ]

    # ---------- День 3 (замены) ----------
    d3_3 = [
        f"Завтрак: йогурт/кефир {yogurt}г + овсянка {oat}г (как мюсли) + ягоды {berries}г",
        f"Обед: макароны {pasta_cooked}г (готовые) + рыба {fish}г + овощи {veg}г + масло {oil}г",
        f"Ужин: картофель {potato}г + курица/индейка {chicken}г + овощи 200г",
    ]
    d3_4 = [
        f"Завтрак: йогурт/кефир {yogurt}г + овсянка {oat}г (как мюсли) + ягоды {berries}г",
        f"Перекус: банан {banana}г + орехи {nuts}г",
        f"Обед: макароны {pasta_cooked}г (готовые) + рыба {fish}г + овощи {veg}г + масло {oil}г",
        f"Ужин: творог {curd}г + хлеб {bread}г (если нужно добрать)",
    ]
    d3_5 = [
        f"Завтрак: йогурт/кефир {yogurt}г + овсянка {oat}г + ягоды {berries}г",
        f"Перекус: яйца ~{eggs} шт или творог {curd}г (что удобнее)",
        f"Обед: макароны {pasta_cooked}г (готовые) + рыба {fish}г + овощи {veg}г + масло {oil}г",
        f"Перекус: банан {banana}г + молоко/кефир {milk}мл",
        f"Ужин: картофель {potato}г + курица/индейка {chicken}г + овощи 200г",
    ]

    day1 = format_by_meals(d1_3, d1_4, d1_5)
    day2 = format_by_meals(d2_3, d2_4, d2_5)
    day3 = format_by_meals(d3_3, d3_4, d3_5)

    return (
        "📌 Примеры питания на 3 дня (можно крутить по кругу)\n"
        "⚠️ Важно: крупы/макароны указаны как ГОТОВЫЙ вес, овсянка — СУХОЙ вес.\n\n"
        + day_block(1, day1) + "\n\n"
        + day_block(2, day2) + "\n\n"
        + day_block(3, day3) + "\n"
    )
...
# =========================
# ПИТАНИЕ (КБЖУ + ✅ 3-дневные примеры)
# =========================
def generate_nutrition_plan(goal: str, sex: str, age: int, height: int, weight: float, exp: str, freq: int = 3, place: str = "дом") -> str:
    ...
    # ✅ ВОТ ТУТ ИЗМЕНЕНИЕ: вместо одного шаблона — 3 дня питания
    examples_3days = build_three_day_menu(calories, meals)
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        "• питание (ккал/БЖУ) + примеры на 3 дня\n"
        "• дневник тренировок\n"
        "• замеры прогресса\n\n"
        "Порядок:\n"
        "1) ⚙️ Профиль\n"
