...
def workout_days_kb(freq: int, has_full_access: bool = False):
    ...
    if has_full_access:
        rows.append([InlineKeyboardButton(text="🔄 Сменить программу", callback_data="workout:rebuild")])
...
def build_day_display_text(day_num: int, day_text: str, exercises: List[str],
    ...
    """Строит текст дня тренировки. БЕЗ калорий."""
    total = len(exercises)
    done_count = len(done)
    ...
    # Список упражнений с галочками (без ккал)
...
def generate_workout_plan(goal: str, place: str, exp: str, freq: int, limits: str, user_id: int = 0) -> Tuple[str, dict]:
    ...
    if is_gym:
        ...
        HPULL = [
            ...
            "Горизонтальные подтягивания (под столом)",
