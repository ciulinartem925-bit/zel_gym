...
class ProfileWizard(StatesGroup):
    goal = State()
    sex = State()
    age = State()       # теперь вводом
    height = State()    # теперь вводом
    weight = State()    # теперь вводом
    place = State()
    exp = State()
    freq = State()
    limits = State()    # ограничения (в конце)
...
TECH = {
    "squat": {
        ...
        "text": (
            ...
            "4) Глубина — пока спина ровная.\n\n"
            "Вверх:\n"
            "1) Отталкивайся серединой стопы.\n"
            "2) Корпус жёсткий.\n\n"
            "Ошибки:\n"
            "• колени внутрь\n"
            "• пятки отрываются\n"
    ...
    "bench": {
        ...
        "text": (
            ...
            "Опускание:\n"
            "1) Гриф к нижней части груди.\n"
            "2) Локти ~45° к корпусу.\n\n"
            "Жим:\n"
            "1) Вверх и чуть назад к стойкам.\n\n"
            "Ошибки:\n"
            "• локти в стороны\n"
            "• ягодицы отрываются\n"
        )
    ...
    "row": {
        ...
        "text": (
            ...
            "2) Начни лопатками → затем локти назад.\n"
            "3) Без раскачки.\n"
        )
    },
    "latpulldown": {
    ...
    "pullup": {
        ...
        "text": (
            ...
            "1) Лопатки вниз → затем тяни локти к рёбрам.\n"
            "2) Вниз медленно.\n"
        )
    },
    "rdl": {
        ...
        "text": (
            ...
            "2) Таз назад, гриф близко к ногам.\n"
            "3) Спина ровная.\n"
        )
    },
    "ohp": {
...
def _bar(step: int, total_steps: int = TOTAL_PROFILE_STEPS) -> str:
    step = max(0, min(step, total_steps))
    pct = int(round(step / total_steps * 100))
    done = int(round(BAR_LEN * (pct / 100.0)))
    done = max(0, min(done, BAR_LEN))
    return f"{'■' * done}{'□' * (BAR_LEN - done)} {pct}%"


def _profile_header(step: int) -> str:
    return f"🧩 Заполнение профиля — {_bar(step)}\n\n"
...
def kb_back_only(step: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ Назад", callback_data=f"p:back:{step}")],
        [InlineKeyboardButton(text="🏠 Меню", callback_data="nav:menu")],
    ])
...
async def init_db():
    async with db() as conn:
        ...
        """)
        # миграция для старых баз: добавим limits, если не было
        try:
...
async def get_measures_history_all(user_id: int, limit_each: int = 6):
    types = ["weight", "waist", "arm", "chest", "thigh"]
    out = {}
    for t in types:
        out[t] = await get_last_measures(user_id, t, limit_each)
    return out
...
# =========================
# ✅ ТРЕНИРОВКИ — формат: "День N" + пункты •
# + индивидуализация под цель/место/опыт/частоту/ограничения
# =========================
...
def _constraints_map(limits: str) -> dict:
    t = (limits or "").lower()
    has = lambda *keys: any(k in t for k in keys)

    return {
        "knee": has("колен", "мениск", "крест", "сустав", "колено"),
        "back": has("поясн", "спин", "грыж", "протруз", "радик"),
        "shoulder": has("плеч", "импиндж", "лопат", "ротатор"),
        "bp": has("давлен", "гиперт", "сердц"),
        "wrist": has("кист", "запяст"),
        "none": (t.strip() in ("нет", "no", "-", "нету", "нет.", "none", "н"))
    }
...
def generate_workout_plan(goal: str, place: str, exp: str, freq: int, limits: str, user_id: int = 0) -> str:
    ...
    cons = _constraints_map(limits)
    lim_line = ""
    if limits and not cons["none"]:
        lim_line = f"Ограничения: {limits.strip()}\n"
    ...
    # ограничения → убираем рискованные варианты
    if cons["knee"]:
        legs_base = [x for x in legs_base if "Присед" not in x and "Жим ногами" not in x]
        legs_base += (["Ягодичный мост", "Румынская тяга (лёгкая)"] if is_gym else ["Ягодичный мост", "Румынская тяга (если есть гантели)"])
    if cons["back"]:
        legs_base = [x for x in legs_base if "Румын" not in x and "наклон" not in x and "Присед со штангой" not in x]
        legs_base += (["Жим ногами (лёгко/контроль)", "Разгибания ног (лёгко)"] if is_gym else ["Приседания (мелко/контроль)", "Ягодичный мост"])
    if cons["shoulder"]:
        push_base = [x for x in push_base if "Жим вверх" not in x]
        shoulders_iso = [x for x in shoulders_iso if "Разведения" in x]  # оставим более мягкий вариант
    # (давление/сердце) → просто напоминание, нагрузку пользователь сам контролит
    ...
    # индивидуально под частоту: на 3 дня меньше объёма, на 5 — больше
    add_core = (f >= 5)
    add_legs_iso = (f >= 4)
    ...
    for d in range(f):
        ...
        lines = []
        lines.append(f"• {push} — {base_sets}×{reps_base}")
        lines.append(f"• {pull} — {base_sets}×{reps_base}")
        lines.append(f"• {legs} — {base_sets}×{reps_base}")
        lines.append(f"• {sh} — {iso_sets}×{reps_iso}")
        lines.append(f"• {bi} — {iso_sets}×{reps_iso}")
        lines.append(f"• {tri} — {iso_sets}×{reps_iso}")
        if add_legs_iso:
...
async def cmd_start(message: Message, bot: Bot):
    ...
    # ✅ ставим клавиатуру снизу, но НЕ оставляем отдельное сообщение в чате
...
async def cb_profile_back(callback: CallbackQuery, state: FSMContext):
    ...
    if step == "goal":
        ...
        await clean_edit(callback, uid, text, reply_markup=kb_back_only("sex"))

    elif step == "height":
        await state.set_state(ProfileWizard.height)
        text = _profile_header(4) + "📏 Введи рост в см (например 178):"
        await clean_edit(callback, uid, text, reply_markup=kb_back_only("age"))

    elif step == "weight":
        await state.set_state(ProfileWizard.weight)
        text = _profile_header(5) + "⚖️ Введи вес в кг (например 74.5):"
        await clean_edit(callback, uid, text, reply_markup=kb_back_only("height"))
...
async def cb_profile_sex(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(ProfileWizard.age)
    text = _profile_header(3) + "🎂 Введи возраст числом (например 22):"
    await clean_edit(callback, callback.from_user.id, text, reply_markup=kb_back_only("sex"))
    await callback.answer()
...
async def profile_age_input(message: Message, state: FSMContext, bot: Bot):
    ...
    age = int(txt)
    if age < 12 or age > 90:
        await clean_send(bot, message.chat.id, message.from_user.id, "Возраст должен быть в диапазоне 12–90.")
        await try_delete_user_message(bot, message)
        return
    ...
    text = _profile_header(4) + "📏 Введи рост в см (например 178):"
    await clean_send(bot, message.chat.id, message.from_user.id, text, reply_markup=kb_back_only("age"))
    await try_delete_user_message(bot, message)
...
async def profile_height_input(message: Message, state: FSMContext, bot: Bot):
    txt = re.sub(r"[^\d]", "", message.text or "")
    if not txt:
        await clean_send(bot, message.chat.id, message.from_user.id, "Рост числом (см). Например 178")
        await try_delete_user_message(bot, message)
        return
    h = int(txt)
    if h < 120 or h > 230:
    ...
    text = _profile_header(5) + "⚖️ Введи вес в кг (например 74.5):"
    await clean_send(bot, message.chat.id, message.from_user.id, text, reply_markup=kb_back_only("height"))
    await try_delete_user_message(bot, message)
...
async def profile_weight_input(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        w = float(re.sub(r"[^\d\.]", "", txt))
    except Exception:
        w = 0.0
    if w <= 0:
        await clean_send(bot, message.chat.id, message.from_user.id, "Вес числом. Например 74.5")
        await try_delete_user_message(bot, message)
        return
    if w < 30 or w > 250:
...
async def cb_profile_exp(callback: CallbackQuery, state: FSMContext):
    ...
    if v == "0":
        ...
        text = (
            ...
            "Например: колени, спина, плечо, давление и т.д.\n\n"
            "Если нет — напиши: нет"
        )
        await clean_edit(callback, callback.from_user.id, text, reply_markup=kb_back_only("exp"))
        await callback.answer()
        return
...
async def cb_profile_freq(callback: CallbackQuery, state: FSMContext):
    ...
    await clean_edit(callback, callback.from_user.id, text, reply_markup=kb_back_only("freq"))
    await callback.answer()
...
async def profile_limits_input(message: Message, state: FSMContext, bot: Bot):
    ...
    if not txt:
        await clean_send(bot, message.chat.id, message.from_user.id, "Напиши ограничения или 'нет'.")
        await try_delete_user_message(bot, message)
        return

    await update_user(message.from_user.id, limits=txt)
    await state.clear()
...
# ✅ Дневник: только ключевые упражнения (для прогресса), 2 столбца, без смайлов (кроме истории)
DIARY_EXERCISES = [
    "Жим лёжа",
    "Присед",
    "Тяга (гребля)",
...
def diary_exercises_kb():
    rows = []
    # 2 столбца
    for i in range(0, len(DIARY_EXERCISES), 2):
        row = [InlineKeyboardButton(text=DIARY_EXERCISES[i], callback_data=f"d:ex:{DIARY_EXERCISES[i]}")]
        if i + 1 < len(DIARY_EXERCISES):
            row.append(InlineKeyboardButton(text=DIARY_EXERCISES[i+1], callback_data=f"d:ex:{DIARY_EXERCISES[i+1]}"))
        rows.append(row)
...
def measures_kb():
    # ✅ замеры как “дневник”: выбираешь что записать, есть история, всё кнопками
    return InlineKeyboardMarkup(inline_keyboard=[
...
def measures_history_back_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ Назад к замерам", callback_data="m:back")],
        [InlineKeyboardButton(text="🏠 Меню", callback_data="nav:menu")],
    ])
...
async def open_measures(user_id: int, chat_id: int, bot: Bot, state: FSMContext, callback: Optional[CallbackQuery] = None):
    ...
    text = (
        "📏 Замеры\n\n"
        "Выбери, что записать.\n"
        "Дата ставится автоматически — тебе нужно только значение."
    )
    if callback:
...
async def cb_measure_type(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        f"Введи значение для «{name}» числом (например 72.5):"
    )
    await callback.answer()
...
async def measure_value(message: Message, state: FSMContext, bot: Bot):
    ...
    out = (
        ...
        f"🗓 {datetime.now().strftime('%Y-%m-%d')}\n\n"
        "Можешь выбрать следующий замер кнопкой «📏 Замеры» или открыть «📜 История»."
    )
...
async def cb_measures_history(callback: CallbackQuery, bot: Bot):
    ...
    hist = await get_measures_history_all(callback.from_user.id, 6)
    names = {"weight": "Вес", "waist": "Талия", "arm": "Рука", "chest": "Грудь", "thigh": "Бедро"}

    lines = ["📜 История замеров\n"]
    for t, rows in hist.items():
        lines.append(f"{names.get(t, t)}:")
        if not rows:
    ...
    await clean_edit(callback, callback.from_user.id, "\n".join(lines).strip(), reply_markup=measures_history_back_kb())
    await callback.answer()

async def cb_measures_back(callback: CallbackQuery, state: FSMContext, bot: Bot):
    await open_measures(callback.from_user.id, callback.message.chat.id, bot, state=state, callback=callback)
    await callback.answer()
...
# =========================
# ✅ ПОСТЫ С КАРТИНКАМИ (АДМИН) — без изменений
# =========================
def admin_posts_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
...
def setup_handlers(dp: Dispatcher):
    ...
    dp.callback_query.register(cb_measures_back, F.data == "m:back")
